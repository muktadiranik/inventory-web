...
const UpdateProduct = ({ productObject }) => {
  ...
  const onSubmit = (event) => {
    ...
    let promise = new Promise((resolve, reject) => {
      dispatch(
        updateProductAction(company.id, token, formData, productObject.id)
      );
      resolve();
    });
    promise
  ...
  useEffect(() => {
    if (productCategoryList.length > 0) {
      setCategory(productCategoryList[0].id);
    }
    setProduct(productObject.name);
    setCategory(productObject.category);
    setProductDescription(productObject.description);
    setUnit(productObject.unit);
    setPrice(productObject.price);
    setStock(productObject.stock);
  }, [productCategoryList, productObject]);
